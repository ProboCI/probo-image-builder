...
build_ubuntu-16.04-lamp-latest () {
	...
	$PACKER build ubuntu-16.04-lamp.json
	echo -e "${INFO}Building mbagnall/ubuntu-16.04:latest image...${NC}"
	docker build --compress -t="mbagnall/ubuntu-16.04:latest" .
	echo -e "${INFO}Image mbagnall/ubuntu-16.04:latest built.${NC}"
}

build_ubuntu-16.04-dotnet-sdk1.0.1 () {
	cd $BUILDER/16.04-dotnetsdk1.0.1
	$PACKER build ubuntu-16.04-dotnet.json
	echo -e "${INFO}Building mbagnall/ubuntu-16.04-dotnet:sdk1.0.1...${NC}"
	docker build --compress -t="mbagnall/ubuntu-16.04-dotnet:sdk1.0.1" .
	echo -e "${INFO}Image mbagnall/ubuntu-16.04-dotnet:sdk1.0.1 built.${NC}"
}

build_ubuntu-16.04-dotnet-sdk2.0.2 () {
	cd $BUILDER/16.04-dotnetsdk2.0.2
	$PACKER build ubuntu-16.04-dotnet.json
	echo -e "${INFO}Building mbagnall/ubuntu-16.04-dotnet:sdk2.0.2...${NC}"
	docker build --compress -t="mbagnall/ubuntu-16.04-dotnet:sdk2.0.2" .
	echo -e "${INFO}Image mbagnall/ubuntu-16.04-dotnet:sdk2.0.2 built.${NC}"
}
